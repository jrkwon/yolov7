...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        if self.kpt_label:
            gain = torch.ones(41, device=targets.device, dtype=torch.long)  # normalized to gridspace gain
        else:
            gain = torch.ones(7, device=targets.device, dtype=torch.long)  # normalized to gridspace gain
